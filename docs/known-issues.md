...
- ~~**补贴报表售后不冲减**~~（✅ 2026-09-16 已修复并验证：两个补贴统计 SQL 加 `NOT EXISTS order_after_sale status=3` 剔除退款完成订单（已拒绝/待处理仍计入，钱未退）；GMV 维持不冲减口径（卡片文案已区分）。实测 36→21 / 店铺 15→10，删售后单复原。精确逐单对账的 mk_subsidy_ledger 流水仍预留未实施，如需冲销明细再上）
